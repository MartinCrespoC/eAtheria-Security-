...
async function main() {
  ...
  const trueCases = csv.split("\n")
    .filter(l => l.includes(",xss,true,"))
    .map(l => l.split(",")[0]);

  console.log(`Testing ${trueCases.length} xss TRUE cases for FNs...`);
  ...
  console.log(`\nFN cases (first 15): ${fns.slice(0, 15).join(", ")}`);
